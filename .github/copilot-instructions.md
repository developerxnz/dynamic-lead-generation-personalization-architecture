...
Customer Session / Trigger
  ...
  -> Contentful / Offer Catalog Query
...
- `docs/services/05-contentful-integration.md` and `docs/architecture/04-system-architecture.md` make the system boundary explicit: Contentful owns managed content and offer metadata, while decisioning logic stays in backend services.
...
- Preserve the separation of concerns used throughout the docs: Contentful/offer catalog content is metadata and managed copy; backend services own intent evaluation, eligibility, suitability, ranking, and business constraints.
- Prefer **broad candidate retrieval plus downstream filtering/ranking** over encoding heavy business logic in Contentful queries. Hard constraints like unpublished content, expired offers, unsupported regions, or missing compliance approval can be filtered early.
