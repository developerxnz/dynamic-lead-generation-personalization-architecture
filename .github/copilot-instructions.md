...
- `docs/services/05-activity-metadata.md` and `docs/architecture/04-system-architecture.md` make the system boundary explicit: existing activities carry the metadata used for retrieval, while decisioning logic stays in backend services.
...
- Preserve the separation of concerns used throughout the docs: activity metadata describes what can be shown; backend services own intent evaluation, eligibility, suitability, ranking, and business constraints.
- Prefer **broad candidate retrieval plus downstream filtering/ranking** over encoding heavy business logic in activity metadata filters. Hard constraints like inactive activities, expired offers, unsupported regions, or missing compliance approval can be filtered early.
